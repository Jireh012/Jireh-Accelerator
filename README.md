...
一个原生 Rust 的 Jireh 专属加速器，提供 **CLI + 桌面 GUI** 双形态。
...
[Star History Chart](https://www.star-history.com/#fjh1997/Linux.do-Accelerator&Date)

## Overview

`jireh-accelerator` 的目标很直接：
...
本项目基于 L站开源项目 [https://github.com/fjh1997/Linux.do-Accelerator](https://github.com/fjh1997/Linux.do-Accelerator) 进行修改，添加了作者想要加速的站点

基于`linux.do 类似的站点` 实际上是被 `SNI` 阻断的。运营商检测到 `linux.do` 的 `SNI` 后，会直接发出 `RST` 包，导致连接被重置。
...
- 配置项集中在单个 `jireh-accelerator.toml`
...
```bash
cargo run --bin jireh-accelerator -- init-config
```

准备证书和 `hosts`：

```bash
sudo cargo run --bin jireh-accelerator -- setup
```

仅准备 `hosts` 规则：

```bash
sudo cargo run --bin jireh-accelerator -- apply-hosts
```

手动确保首次 `hosts` 基线备份存在：

```bash
cargo run --bin jireh-accelerator -- backup-hosts
```

恢复到首次接管前的 `hosts` 完整备份：

```bash
sudo cargo run --bin jireh-accelerator -- restore-hosts
```
...
```bash
sudo cargo run --bin jireh-accelerator -- start
```

停止后台加速：

```bash
sudo cargo run --bin jireh-accelerator -- stop
```

查看当前状态：

```bash
cargo run --bin jireh-accelerator -- status
```

直接打开 GUI：

```bash
cargo run --bin jireh-accelerator
```
...
默认情况下，程序只使用一个主配置文件 `jireh-accelerator.toml`。


| 平台      | 主配置文件                                                                                   | 运行状态目录                                                             | 证书目录                                                             |
| ------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | ---------------------------------------------------------------- |
| Linux   | `~/.config/jireh-accelerator/jireh-accelerator.toml`                                    | `~/.local/share/jireh-accelerator/runtime`                         | `~/.local/share/jireh-accelerator/certs`                         |
| Windows | `%APPDATA%\jireh\jireh-accelerator\config\jireh-accelerator.toml`                       | `%LOCALAPPDATA%\jireh\jireh-accelerator\data\runtime`              | `%LOCALAPPDATA%\jireh\jireh-accelerator\data\certs`              |
| macOS   | `~/Library/Application Support/io.jireh.jireh-accelerator/jireh-accelerator.toml`       | `~/Library/Application Support/io.jireh.jireh-accelerator/runtime` | `~/Library/Application Support/io.jireh.jireh-accelerator/certs` |
| Android | `/storage/emulated/0/Android/media/io.jireh.accelerator.android/jireh-accelerator.toml` | Android VPN 运行时内存态                                                 | 非 Root 版无需证书                                                     |


如果显式指定：

```bash
jireh-accelerator --config /path/to/jireh-accelerator.toml
```
...
- `jireh-accelerator-android-arm64-v8a.apk`
- `jireh-accelerator-android-x86_64.apk`
...
- `clean-hosts` 只删除 `jireh-accelerator` 自己维护的 marker block
